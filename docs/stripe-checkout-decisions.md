...
| line_items | `price_data` dynamique : libellé « Lecture symbolique personnalisée (Lastro) », **prix fixe calculé par le serveur** — 25 € , ou 15 € avec le code de lancement `bessbousse10` (voir [src/payments/pricing.mjs](src/payments/pricing.mjs), convention `lastro-pricing@1.0.0`), devise `STRIPE_CURRENCY` (eur) | **Rien à faire** : le prix est décidé par le serveur et il n'y a aucun `price_...` à créer dans le Dashboard. Pour changer le prix ou l'offre : `ASTROLAB_PRICE_CENTS`, `ASTROLAB_PROMO_CODE`, `ASTROLAB_PROMO_DISCOUNT_CENTS` (voir [.env.example](.env.example)). Le navigateur n'envoie jamais de montant : il n'envoie qu'un code, et un code inconnu est refusé (400) au lieu d'être facturé au plein tarif. |
